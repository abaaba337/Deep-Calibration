...
class GridbasedFNN(nn.Module): 
    ...
    def __init__(self, loader):
        ...
        self.hidden1 = nn.Sequential( nn.Linear(6,30) , nn.Sigmoid() )
        self.hidden2 = nn.Sequential( nn.Linear(30,30), nn.Sigmoid() )
    ...
    ## Forward function
    def forward(self, x):         
        x = self.hidden1(x)      # x : [batch_size, 6 ] ---> [batch_size, 30] , activation
        x = self.hidden2(x)      # x : [batch_size, 30] ---> [batch_size, 30] , activation
        x = self.hidden2(x)      # x : [batch_size, 30] ---> [batch_size, 30] , activation
        x = self.output(x)       # x : [batch_size, 30] ---> [batch_size, 88 ]
        return x
    ...
    def predict(self, x):
        ...
        with torch.no_grad():
            ...
            y_hat = self.forward(x).squeeze().numpy()
            return y_hat
